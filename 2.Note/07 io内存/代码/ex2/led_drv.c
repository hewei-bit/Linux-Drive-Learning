...
ssize_t led_write (struct file *file, const char __user *buf, size_t len, loff_t *offset)
{
	...
	switch(kbuf[0])
	{
		case 7://D7灯的亮与灭
		{
			if(kbuf[1] == 1)
			{
				// (*(volatile unsigned int *)gpioe_out_va)&=~(1<<13);
				value = ioread32(gpioe_out_va);
				value &=~(1<<13);
				iowrire32(value , gpioe_out_va );
			}
