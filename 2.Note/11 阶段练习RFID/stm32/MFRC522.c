...
{ 
	...
	//配置端口 PC6 PC11为输出模式
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_11;	//第6 11
	...
	//PD6 PD7 配置输出模式
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_7;	//第6 7
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;			//输出模式
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;			//推挽输出，增强驱动能力，引脚的输出电流更大
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;		//引脚的速度最大为100MHz
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;		//没有使用内部上拉电阻
	...
	//配置端口 PC8为输入模式
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;				//第8
