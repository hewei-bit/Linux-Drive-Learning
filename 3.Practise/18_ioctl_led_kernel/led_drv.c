...
long led_ioctl (struct file *filp, unsigned int cmd, unsigned long args)
{
	...
	switch(cmd)
	{
		case GEC6818_LED_ON:
		{
#if CONFIG_GEC6818_LED_N	==	7
			if(arg ==  7)gpio_set_value(PAD_GPIO_E + 13,0);
#endif

#if CONFIG_GEC6818_LED_N	==	8
			if(arg ==  8)gpio_set_value(PAD_GPIO_C + 17,0);
#endif			

#if CONFIG_GEC6818_LED_N	==	9
			if(arg ==  9)gpio_set_value(PAD_GPIO_C + 8,0);
#endif

#if CONFIG_GEC6818_LED_N	==	10
			if(arg == 10)gpio_set_value(PAD_GPIO_C + 7,0);		
#endif			
		}break;
		...
		{
#if CONFIG_GEC6818_LED_N	==	7
			if(arg ==  7)gpio_set_value(PAD_GPIO_E + 13,1);
#endif

#if CONFIG_GEC6818_LED_N	==	8
			if(arg ==  8)gpio_set_value(PAD_GPIO_C + 17,1);
#endif			

#if CONFIG_GEC6818_LED_N	==	9
			if(arg ==  9)gpio_set_value(PAD_GPIO_C + 8,1);
#endif	

#if CONFIG_GEC6818_LED_N	==	10
			if(arg == 10)gpio_set_value(PAD_GPIO_C + 7,1);		
#endif		
		}break;	
